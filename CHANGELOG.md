...
### رفع مشکل
- **رفع خطای ۴۰۴ صفحه اتصالات:** تغییر اولویت لود منوی اتصالات به اولویت ۱۲ جهت اطمینان از ثبت کامل منوی مادر (تنظیمات بایروز) پیش از زیرمنو.

## [4.10.1] - 2025-07-14

### رفع مشکل
- غیرفعال‌سازی موقت ماژول «رفع دسترسی‌پذیری» به‌دلیل مشکل نمایش صفحه محصول (متن‌ها عمودی نمایش داده می‌شد). ماژول در نسخه آینده با رویکرد جدید بازفعال خواهد شد.

## [4.10.0] - 2025-07-14

### ویژگی جدید
- **ماژول «رفع دسترسی‌پذیری» اضافه شد** — اصلاح خودکار مشکلات ARIA و ساختار HTML در صفحه محصول
- **بهبود کنتراست رنگ placeholder ورودی جستجو** برای کاربران کم‌بینا
- **افزودن برچسب‌های ARIA** به نوار پیشرفت سبد خرید، لینک‌های شبکه اجتماعی و دکمه‌های آیکونی
- **اصلاح سطح عناوین صفحه محصول** برای ناوبری بهتر با صفحه‌خوان
- **رفع ساختار نامعتبر لیست‌ها** در جدول مشخصات و سایدبار سبد
- **رفع شناسه تکراری** در فرم اطلاع‌رسانی SMS

### توضیحات
- این ماژول از داشبورد تنظیمات بایروز قابل فعال/غیرفعال‌سازی است.

## [4.9.7] - 2025-07-01

### رفع باگ
- **ترتیب نوار بالا (Admin Bar):** آیتم «تنظیمات بایروز» اکنون بلافاصله بعد از «تنظیمات پوسته» نمایش داده می‌شود — با reorder کردن nodeها در `wp_before_admin_bar_render`.

## [4.9.6] - 2025-07-01

### رفع باگ
- **ترتیب منوی سایدبار:** منوی «تنظیمات بایروز» اکنون بلافاصله بعد از «قالب باکالا» نمایش داده می‌شود (قبلاً بعد از المنتور بود).
- **ترتیب نوار بالا (Admin Bar):** آیتم «تنظیمات بایروز» اکنون بلافاصله بعد از «تنظیمات پوسته» قرار می‌گیرد (قبلاً بعد از Perfmatters بود).

## [4.9.5] - 2026-06-27

### بهبود
- **ترتیب فیلدهای لینک اصلاح شد:** آیکون 🔗 سمت راست، متن انکرتکست وسط، URL سمت چپ — مطابق جهت خوانش RTL.
- **Hover آیتم‌ها ملایم‌تر:** رنگ border هنگام hover از آبی bold به خاکستری ملایم تغییر کرد — حرفه‌ای‌تر و کمتر تهاجمی.

## [4.9.4] - 2026-06-27

### رفع باگ
- **آیکون تب ضمانت:** رنگ، پس‌زمینه، سایز و موقعیت آیکون اصلاح شد تا دقیقاً مطابق تب‌های دیگر باکالا باشد (پس‌زمینه طلایی `accent_color2`، آیکون سفید، ابعاد 35×35، border-radius 10px).

## [4.9.3] - 2026-06-27

### بهبود
- **ترتیب فیلدهای لینک:** متن نمایشی (anchor text) سمت راست و آدرس لینک سمت چپ — مطابق جهت RTL.
- **کنتراست بین آیتم‌ها:** آیتم‌های زوج پس‌زمینه خاکستری ملایم و فاصله بیشتر بین آیتم‌ها — تشخیص مرز هر آکاردئون آسان‌تر.
- **Hover بهتر:** بوردر رنگی و سایه واضح‌تر هنگام hover.

## [4.9.2] - 2026-06-27

### بهبود
- **UI ادمین تب ضمانت:** layout هر آیتم آکاردئون از حالت افقی (inline) به عمودی (stacked) تغییر کرد — عنوان بالا، محتوا زیر، لینک‌ها پایین. خوانایی و مدیریت آسان‌تر.
- **Placeholder بهتر:** فیلد متن لینک حالا مثال عملی نمایش میده (مثال: مطالعه قوانین مرجوعی).
- **تأیید حذف:** قبل از حذف هر آیتم، یک confirm نمایش داده میشه تا از حذف تصادفی جلوگیری بشه.
- **آیکون 🔗 برای فیلدهای لینک:** برای تشخیص سریع‌تر بخش لینک.

## [4.9.1] - 2026-06-27

### بهبود
- **محتوای آکاردئون بدون HTML:** ادمین دیگر نیازی به نوشتن `<br>` ندارد. کافیست در textarea اینتر بزند — شکست خطوط به‌صورت خودکار در فرانت‌اند اعمال می‌شود.
- **Backward compatible:** محتوای قبلی که `<br>` دارد، هنگام ذخیره بعدی به‌صورت خودکار به newline تبدیل می‌شود.
- **امنیت:** محتوا اکنون plaintext ذخیره و با `esc_html` + `nl2br` رندر می‌شود — بدون ریسک XSS.

## [4.9.0] - 2026-06-27

### ویژگی جدید
- **لینک CTA در آکاردئون تب ضمانت:** هر آیتم آکاردئون اکنون دو فیلد اختیاری «لینک» و «متن لینک» دارد. در صورت تکمیل، لینک در انتهای محتوای آکاردئون با `target="_blank"` نمایش داده می‌شود — بدون خارج شدن کاربر از صفحه محصول.

### توضیحات
- لینک‌ها اختیاری‌اند: اگر فیلد URL خالی باشد، هیچ لینکی نمایش داده نمی‌شود.
- لینک‌ها سراسری (Global) هستند و برای همه محصولات یکسان عمل می‌کنند — بدون کوئری اضافه به دیتابیس.
- تیتر آکاردئون لینک‌دار نمی‌شود — فقط toggle باز/بسته شدن آکاردئون.

## [4.8.2] - 2026-06-27

### ویژگی جدید
- **عنوان و توضیحات تب ضمانت در فرانت‌اند:** اکنون هنگام باز کردن تب ضمانت، عنوان تب (h2) و توضیحات زیرش نمایش داده می‌شود — مشابه ساختار تب مشخصات باکالا.
- **فیلد توضیحات تب در ادمین:** فیلد جدید «توضیحات تب» به صفحه تنظیمات ماژول اضافه شد. متن وارد شده زیر عنوان تب در صفحه محصول نمایش داده می‌شود.

## [4.8.1] - 2026-06-27

### بهبود
- **آیکون تب ضمانت محصول:** آیکون تب از emoji به فونت‌آیکون اختصاصی قالب (bakala) تغییر کرد تا با سایر تب‌های صفحه محصول (نقد و بررسی، مشخصات، دیدگاه‌ها) کاملاً هماهنگ و یکدست باشد.
- **عنوان پیش‌فرض تب:** عنوان پیش‌فرض از «🛡️ ضمانت، ارسال و پشتیبانی» به «ضمانت و ارسال» تغییر کرد — کوتاه‌تر و هم‌وزن با عناوین تب‌های دیگر.

### توضیحات
- اگر عنوان تب قبلاً ذخیره شده، تا زمانی که فیلد عنوان را خالی نکنید و ذخیره نزنید، عنوان قدیمی نمایش داده می‌شود.

## [4.8.0] - 2026-06-26

### ویژگی جدید
- **ماژول تب ضمانت محصول:** تب آکاردئونی «ضمانت، ارسال و پشتیبانی» در صفحه محصول ووکامرس. محتوای آکاردئون از پنل ادمین قابل مدیریت است (افزودن، ویرایش، حذف، تغییر ترتیب). عنوان تب قابل تنظیم و تب بین مشخصات و دیدگاه‌ها نمایش داده می‌شود.
- **تنظیمات مشترک آکاردئون:** شعاع گوشه‌ها، اندازه فونت عنوان/پاسخ، نوع آیکون (شورون، بعلاوه/منها، فلش) و فاصله بین آیتم‌ها اکنون از تنظیمات ماژول قابل کنترل هستند و روی همه آکاردئون‌های سایت (FAQ و تب ضمانت) اعمال می‌شوند.

### توضیحات
- ماژول تب ضمانت محصول به‌صورت پیش‌فرض غیرفعال است. از پیشخوان تنظیمات بایروز (بخش ماژول‌ها) آن را فعال کنید.
- تنظیمات ظاهری آکاردئون در صفحه تنظیمات همین ماژول قرار دارند و تغییرات روی تمام آکاردئون‌های سایت تأثیر می‌گذارند.

## [4.6.2] - 2025-07-01

### حذف
- **حذف قابلیت تگ مخفی باکس ارسال رایگان:** قابلیت پشتیبانی از تگ `<span class="sr-only">` در متن باکس ارسال رایگان که در نسخه ۴.۶.۱ اضافه شده بود، به‌طور کامل حذف شد.

## [4.6.0] - 2025-07-01

### ویژگی جدید
- **ماژول ویرایش برچسب‌ها:** امکان جایگزینی متن‌های ترجمه‌شدهٔ تم (مانند پیام‌های موجودی) از پنل تنظیمات بایروز، بدون نیاز به ویرایش فایل‌های تم. از منوی ماژول‌ها فعال کنید و متن دلخواه خود را وارد نمایید.

### توضیحات
- ماژول ویرایش برچسب‌ها به‌صورت پیش‌فرض غیرفعال است. برای استفاده ابتدا از پیشخوان تنظیمات بایروز آن را روشن کنید.

## [4.5.23] - 2026-06-26

### اصلاحات
- **فیلتر هوشمند پاسخ پل آفلاین:** اصلاح بازنویسی ناخواسته ستون‌های SiteToSheet (قیمت ویژه، موجودی، تاریخ‌ها) هنگام ارسال جزئی فیلدها. اکنون تنها فیلدهایی که واقعاً ارسال شده‌اند در پاسخ سایت برگردانده و در شیت نوشته می‌شوند. مثلاً اگر فقط قیمت اصلی ارسال شود، قیمت ویژه و موجودی دیگر با مقدار خالی بازنویسی نمی‌شوند.
- **تبدیل مقادیر عددی قیمت و موجودی:** مقادیر قیمت اصلی، قیمت ویژه و موجودی برگشتی از سایت اکنون به فرمت عدد در شیت ذخیره می‌شوند (به‌جای متن) تا مقایسه و محاسبات شیت به‌درستی انجام شود.

## [4.5.22] - 2026-06-25

### اصلاحات
- **پاک شدن خودکار فیلد متنی ورودی در پل آفلاین:** در صورت موفقیت‌آمیز بودن عملیات همگام‌سازی، زمانی که نیازی به کپی پاسخ سایت نیست، فیلد متنی (textarea) به صورت خودکار پاک می‌شود تا کاربر نیازی به پاک کردن دستی آن نداشته باشد.
- **نمایش هوشمند مودال کپی:** مودال پاسخ تنها در مواردی نمایش داده می‌شود که واقعاً فیلدهایی برای بازگرداندن و نوشتن در گوگل شیت وجود داشته باشند.

## [4.5.21] - 2026-06-25

### اصلاحات
- **بازگرداندن تمامی مقادیر ثبت‌شده محصول به شیت:** اکنون پس از ذخیره محصول در پل آفلاین، محصول مجدداً از دیتابیس بارگذاری می‌شود تا از صحت ثبت قیمت اصلی، قیمت تخفیف، موجودی و تاریخ‌های شگفت‌انگیز اطمینان حاصل شود.
- **ارسال مقادیر کامل برگشتی در نتایج:** نتایج برگشتی از پل آفلاین اکنون شامل مقادیر کامل ذخیره شده بوده و مودال کپی کدها برای انواع بروزرسانی‌های گروهی نمایش داده می‌شود تا در بخش پاسخ سایدبار شیت قرار بگیرد.

## [4.5.20] - 2026-06-13

### اصلاحات
- **پشتیبانی از فرمت ترکیبی صف گوگل شیت در پل آفلاین:** اکنون فرمت JSON خروجی دکمه «کپی صف» شیت (شامل `price_updates` و `brand_sync`) مستقیماً در پل آفلاین قابل paste و اعمال است. سیستم ابتدا برندهای جدید را ایجاد و سپس آپدیت قیمت‌ها را اعمال می‌کند.

## [4.5.18] - 2026-06-09

### اصلاحات
- **بستن خودکار مدال پس از کپی در پل آفلاین:** اضافه شدن انیمیشن باز و بسته شدن (ترانزیشن افکت) به مدال نمایش خروجی پل آفلاین و تغییر منطق دکمه کپی به طوری که پس از اتمام موفقیت‌آمیز کپی، بعد از ۱.۵ ثانیه مدال به صورت خودکار با افکت مناسب بسته می‌شود.

## [4.5.17] - 2026-06-08

### اصلاحات
- **بروزرسانی نامک (اسلاگ) برندهای موجود:** اگر برندی از قبل روی سایت ثبت شده باشد ولی اسلاگ آن با شیت متفاوت باشد (مثلاً نامک فارسی داشته باشد)، اکنون پل آفلاین سایت به طور خودکار اسلاگ سایت را به اسلاگ انگلیسی تعیین‌شده در شیت بروزرسانی می‌کند.
- **بازگرداندن اسلاگ نهایی به شیت:** برای تطابق کامل، پل آفلاین سایت اکنون اسلاگ نهایی ثبت‌شده را به گوگل شیت بازمی‌گرداند تا در شیت نیز درجا ذخیره شود و هیچ نامک خالی یا ناهماهنگی باقی نماند.

## [4.5.16] - 2026-06-08

### اصلاحات
- **پشتیبانی از اسلاگ در ثبت برند جدید آفلاین:** رفع اشکال نادیده گرفتن اسلاگ (نامک انگلیسی) در پل آفلاین سایت که سبب تولید خودکار نامک فارسی می‌شد. اکنون اسلاگ تعیین شده به طور صحیح به تابع `wp_insert_term` پاس داده می‌شود.

## [4.5.5] - 2026-06-01

### افزوده‌ها
- **پشتیبانی کامل از فیلدهای محصولات در پل آفلاین:** امکان اعمال انواع فیلدهای محصولات (شامل نام، نامک، وضعیت، ابعاد، تگ‌ها، دسته‌بندی‌ها، برندها، ویژگی‌ها، تصاویر و کلیدهای متادیتا اختصاصی) فراهم شد تا نیازی به محدود شدن به قیمت و موجودی نباشد.
- **کش کردن و دانلود بهینه تصاویر:** تصاویر فرستاده شده در پل آفلاین به طور خودکار دانلود و کش می‌شوند تا از دانلود فایل‌های تکراری جلوگیری شود.

## [4.2.1] - 2026-05-30
### بهبودهای سیستمی
- **حذف سیستم تشخیص WAF**: با توجه به شناسایی ریشه مشکل (خاموش بودن پیش‌فرض ماژول‌ها و رفتار هسته وردپرس)، منوها و کدهای تشخیص نقطه‌ای که دیگر نیازی به آن‌ها نبود حذف شدند.
- **جلوگیری از خطای 403**: دکمه تنظیمات ماژول در پیشخوان در صورت خاموش بودن ماژول غیرفعال (Unclickable) می‌شود تا از ارجاع کاربر به صفحه ثبت‌نشده جلوگیری کند.
